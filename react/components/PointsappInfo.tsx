...
const PointsappInfo = ({ basePoint, quantityPoint }: Props) => {
  ...
  numberPoint *= quantityPoint

  // returns null when one of the values entered by the user is 0 or negative
  ...
  return (
    <div className={`${handles.container} flex `}>
      {numberPoint > 0 ? (
        <p>Te puedes ganar {numberPoint} puntos con esta compra</p>
      ) : null}
    </div>
  )
}
